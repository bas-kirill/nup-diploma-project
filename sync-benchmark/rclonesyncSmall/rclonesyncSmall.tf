...
resource "aws_iam_role" "bench_role" {
  name               = "rclonesync-small-bench-ec2-role"
...
resource "aws_iam_instance_profile" "bench_profile" {
  name = "rclonesync-small-bench-instance-profile"
  role = aws_iam_role.bench_role.name
}
...
resource "aws_subnet" "workload_public" {
  ...
  tags = {
    Name = "rclonesync-small-public-subnet"
  }
}
...
resource "aws_route_table" "workload_public" {
  ...
  tags = {
    Name = "rclonesync-small-public-rt"
  }
}
...
resource "aws_instance" "workload" {
  ...
  user_data = <<-USERDATA
    ...
    aws s3 cp "s3://diploma-scripts/bootstrapRclonesyncSmall.sh" "/tmp/bootstrapRclonesyncSmall.sh"
    chmod +x "/tmp/bootstrapRclonesyncSmall.sh"
    /tmp/bootstrapRclonesyncSmall.sh
  USERDATA

  tags = {
    Name = "rclonesync-small-benchmark"
  }
}
