...
resource "aws_iam_role" "bench_role" {
  name               = "s3cmdsync-large-bench-ec2-role"
...
resource "aws_iam_instance_profile" "bench_profile" {
  name = "s3cmdsync-large-bench-instance-profile"
  role = aws_iam_role.bench_role.name
}
...
resource "aws_subnet" "workload_public" {
  ...
  tags = {
    Name = "s3cmdsync-large-public-subnet"
  }
}
...
resource "aws_route_table" "workload_public" {
  ...
  tags = {
    Name = "s3cmdsync-large-public-rt"
  }
}
...
resource "aws_instance" "workload" {
  ...
  user_data = <<-USERDATA
    ...
    aws s3 cp "s3://diploma-scripts/bootstrapS3cmdsyncLarge.sh" "/tmp/bootstrapS3cmdsyncLarge.sh"
    chmod +x "/tmp/bootstrapS3cmdsyncLarge.sh"
    /tmp/bootstrapS3cmdsyncLarge.sh
  USERDATA

  tags = {
    Name = "s3cmdsync-large-benchmark"
  }
}
