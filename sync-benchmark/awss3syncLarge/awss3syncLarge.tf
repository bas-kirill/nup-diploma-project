...
resource "aws_iam_role" "bench_role" {
  name               = "awss3sync-large-bench-ec2-role"
...
resource "aws_iam_instance_profile" "bench_profile" {
  name = "awss3sync-large-bench-instance-profile"
  role = aws_iam_role.bench_role.name
}
...
resource "aws_vpc" "workload" {
  ...
  tags = { Name = "awss3sync-large-vpc" }
}

resource "aws_internet_gateway" "workload" {
  vpc_id = aws_vpc.workload.id
  tags   = { Name = "awss3sync-large-igw" }
}
...
resource "aws_subnet" "workload_public" {
  ...
  tags = { Name = "awss3sync-large-public-subnet" }
}

resource "aws_route_table" "workload_public" {
  vpc_id = aws_vpc.workload.id
  tags   = { Name = "awss3sync-large-public-rt" }
}
...
resource "aws_instance" "workload" {
  ...
  user_data = <<-USERDATA
    ...
    aws s3 cp "s3://diploma-scripts/bootstrapAwss3syncLarge.sh" "/tmp/bootstrapAwss3syncLarge.sh"
    chmod +x "/tmp/bootstrapAwss3syncLarge.sh"
    /tmp/bootstrapAwss3syncLarge.sh
  USERDATA

  tags = { Name = "awss3sync-large-benchmark" }
}
