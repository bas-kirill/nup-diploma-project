###############################################################################
#  Terraform — инфраструктура для бенчмарка rclone sync
###############################################################################
...
resource "aws_iam_role" "bench_role" {
  name               = "rclonesync-maxdatasetsize-bench-ec2-role"
...
resource "aws_iam_instance_profile" "bench_profile" {
  name = "rclonesync-maxdatasetsize-bench-instance-profile"
  role = aws_iam_role.bench_role.name
}
...
resource "aws_vpc" "workload" {
  ...
  tags = {
    Name = "rclonesync-maxdatasetsize-vpc"
  }
}
...
resource "aws_internet_gateway" "workload" {
  ...
  tags = {
    Name = "rclonesync-maxdatasetsize-igw"
  }
}
...
resource "aws_subnet" "workload_public" {
  ...
  tags = {
    Name = "rclonesync-maxdatasetsize-public-subnet"
  }
}
...
resource "aws_route_table" "workload_public" {
  ...
  tags = {
    Name = "rclonesync-maxdatasetsize-public-rt"
  }
}
...
resource "aws_security_group" "workload_sg" {
  name        = "rclonesync-maxdatasetsize-sg"
  vpc_id      = aws_vpc.workload.id
  description = "Allow all ingress/egress (rclone sync benchmark)"
...
resource "aws_cloudwatch_log_group" "workload_logs" {
  name              = "/bench/rclonesync-maxdatasetsize"
...
resource "aws_instance" "workload" {
  ...
  user_data = <<-USERDATA
    ...
    aws s3 cp "s3://diploma-scripts/bootstrapRclonesyncMaxDatasetSize.sh" "/tmp/bootstrapRclonesyncMaxDatasetSize.sh"
    chmod +x "/tmp/bootstrapRclonesyncMaxDatasetSize.sh"
    /tmp/bootstrapRclonesyncMaxDatasetSize.sh
  USERDATA

  tags = {
    Name = "rclonesync-maxdatasetsize-benchmark"
  }
}
