...
S3_BUCKET_SANDBOX="s3://diploma-s3cmdsync-maxdatasetsize-benchmark"
...
s3cmdsyncMaxDatasetSize() {
  ...
  s3cmd sync "/tmp/prewarming/" "$S3_BUCKET_SANDBOX" \
    --no-preserve \
    --check-md5 \
  ...
  local perf_folder="$base_dir/s3cmdsync/$scenario_name"
  mkdir -p "$perf_folder"
  ...
  for i in {1..10}; do
    ...
    local perf_data_file="$perf_folder/s3cmdsync.$scenario_name.$i.perf.data"
    local start_time=$(date +%s.%N)
    perf record -F 99 -g --call-graph=dwarf -o "$perf_data_file" -- \
      s3cmd sync "$datasets_folder" "$S3_BUCKET_SANDBOX" \
        --no-preserve \
        --check-md5 \
        --multipart-chunk-size-mb=64
    ...
    local single_report="$perf_folder/s3cmdsync.$scenario_name.$i.report.md"
  ...
  local report_file="$perf_folder/s3cmdsync.$scenario_name.report.md"
  ...
  for f in "$perf_folder"/s3cmdsync."$scenario_name".*."perf.data"; do
    perf script -i "$f" >> "$perf_folder/all.stacks"
  done

  ./FlameGraph/stackcollapse-perf.pl "$perf_folder/all.stacks" > "$perf_folder/all.folded"
  flamegraph_file="$perf_folder/s3cmdsync.$scenario_name.flamegraph.html"
