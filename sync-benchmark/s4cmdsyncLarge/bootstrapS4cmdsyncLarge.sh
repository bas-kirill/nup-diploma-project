        ...
        "collect_list": [
          {
            ...
            "log_group_name": "/bench/s4cmdsync-large",
...
# Benchmark scripts
mkdir -p /opt/bench/s4cmdsyncLarge
aws s3 cp s3://diploma-scripts/benchmarkLarge.sh /opt/bench/benchmarkLarge.sh
aws s3 cp s3://diploma-scripts/s4cmdsyncLarge.sh /opt/bench/s4cmdsyncLarge/s4cmdsyncLarge.sh

chmod +x /opt/bench/benchmarkLarge.sh
chmod +x /opt/bench/s4cmdsyncLarge/s4cmdsyncLarge.sh

cd /opt/bench
sudo -E ./benchmarkLarge.sh s4cmdsyncLarge
