...
resource "aws_iam_role" "bench_role" {
  name               = "s4cmdsync-large-bench-ec2-role"
...
resource "aws_iam_instance_profile" "bench_profile" {
  name = "s4cmdsync-large-bench-instance-profile"
  role = aws_iam_role.bench_role.name
}
...
resource "aws_subnet" "workload_public" {
  ...
  tags = {
    Name = "s4cmdsync-large-public-subnet"
  }
}
...
resource "aws_route_table" "workload_public" {
  ...
  tags = {
    Name = "s4cmdsync-large-public-rt"
  }
}
...
resource "aws_instance" "workload" {
  ...
  user_data = <<-USERDATA
    ...
    aws s3 cp "s3://diploma-scripts/bootstrapS4cmdsyncLarge.sh" "/tmp/bootstrapS4cmdsyncLarge.sh"
    chmod +x "/tmp/bootstrapS4cmdsyncLarge.sh"
    /tmp/bootstrapS4cmdsyncLarge.sh
  USERDATA

  tags = {
    Name = "s4cmdsync-large-benchmark"
  }
}
