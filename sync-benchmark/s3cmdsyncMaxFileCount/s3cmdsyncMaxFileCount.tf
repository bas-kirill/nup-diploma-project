...
resource "aws_iam_role" "bench_role" {
  name               = "s3cmdsync-maxfilecount-bench-ec2-role"
...
resource "aws_iam_instance_profile" "bench_profile" {
  name = "s3cmdsync-maxfilecount-bench-instance-profile"
  role = aws_iam_role.bench_role.name
}
...
resource "aws_vpc" "workload" {
  ...
  tags = {
    Name = "s3cmdsync-maxfilecount-vpc"
  }
}
...
resource "aws_internet_gateway" "workload" {
  ...
  tags = {
    Name = "s3cmdsync-maxfilecount-igw"
  }
}
...
resource "aws_subnet" "workload_public" {
  ...
  tags = {
    Name = "s3cmdsync-maxfilecount-public-subnet"
  }
}
...
resource "aws_route_table" "workload_public" {
  ...
  tags = {
    Name = "s3cmdsync-maxfilecount-public-rt"
  }
}
...
resource "aws_security_group" "workload_sg" {
  name        = "s3cmdsync-maxfilecount-sg"
  vpc_id      = aws_vpc.workload.id
...
resource "aws_cloudwatch_log_group" "workload_logs" {
  name              = "/bench/s3cmdsync-maxfilecount"
...
resource "aws_instance" "workload" {
  ...
  user_data = <<-USERDATA
    ...
    aws s3 cp "s3://diploma-scripts/bootstrapS3cmdsyncMaxFileCount.sh" "/tmp/bootstrapS3cmdsyncMaxFileCount.sh"
    chmod +x "/tmp/bootstrapS3cmdsyncMaxFileCount.sh"
    /tmp/bootstrapS3cmdsyncMaxFileCount.sh
  USERDATA

  tags = {
    Name = "s3cmdsync-maxfilecount-benchmark"
  }
}
