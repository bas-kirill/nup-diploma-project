...
resource "aws_iam_role" "bench_role" {
  name               = "mcmirror-maxdatasetsize-bench-ec2-role"
...
resource "aws_iam_instance_profile" "bench_profile" {
  name = "mcmirror-maxdatasetsize-bench-instance-profile"
  role = aws_iam_role.bench_role.name
}
...
resource "aws_vpc" "workload" {
  ...
  tags = {
    Name = "mcmirror-maxdatasetsize-vpc"
  }
}
...
resource "aws_internet_gateway" "workload" {
  ...
  tags = {
    Name = "mcmirror-maxdatasetsize-igw"
  }
}
...
resource "aws_subnet" "workload_public" {
  ...
  tags = {
    Name = "mcmirror-maxdatasetsize-public-subnet"
  }
}
...
resource "aws_route_table" "workload_public" {
  ...
  tags = {
    Name = "mcmirror-maxdatasetsize-public-rt"
  }
}
...
resource "aws_security_group" "workload_sg" {
  name        = "mcmirror-maxdatasetsize-sg"
  vpc_id      = aws_vpc.workload.id
...
resource "aws_cloudwatch_log_group" "workload_logs" {
  name              = "/bench/mcmirror-maxdatasetsize"
...
resource "aws_instance" "workload" {
  ...
  user_data = <<-USERDATA
    ...
    aws s3 cp "s3://diploma-scripts/bootstrapMcmirrorMaxDatasetSize.sh" "/tmp/bootstrapMcmirrorMaxDatasetSize.sh"
    chmod +x "/tmp/bootstrapMcmirrorMaxDatasetSize.sh"
    /tmp/bootstrapMcmirrorMaxDatasetSize.sh
  USERDATA

  tags = {
    Name = "mcmirror-maxdatasetsize-benchmark"
  }
}
