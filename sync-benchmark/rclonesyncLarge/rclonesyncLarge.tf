...
resource "aws_iam_role" "bench_role" {
  name               = "rclonesync-large-bench-ec2-role"
...
resource "aws_iam_instance_profile" "bench_profile" {
  name = "rclonesync-large-bench-instance-profile"
  role = aws_iam_role.bench_role.name
}
...
resource "aws_subnet" "workload_public" {
  ...
  tags = {
    Name = "rclonesync-large-public-subnet"
  }
}
...
resource "aws_route_table" "workload_public" {
  ...
  tags = {
    Name = "rclonesync-large-public-rt"
  }
}
...
resource "aws_instance" "workload" {
  ...
  user_data = <<-USERDATA
    ...
    aws s3 cp "s3://diploma-scripts/bootstrapRclonesyncLarge.sh" "/tmp/bootstrapRclonesyncLarge.sh"
    chmod +x "/tmp/bootstrapRclonesyncLarge.sh"
    /tmp/bootstrapRclonesyncLarge.sh
  USERDATA

  tags = {
    Name = "rclonesync-large-benchmark"
  }
}
