...
resource "aws_iam_role" "bench_role" {
  name               = "mcmirror-small-bench-ec2-role"
...
resource "aws_iam_instance_profile" "bench_profile" {
  name = "mcmirror-small-bench-instance-profile"
  role = aws_iam_role.bench_role.name
}
...
resource "aws_subnet" "workload_public" {
  ...
  tags = {
    Name = "mcmirror-small-public-subnet"
  }
}
...
resource "aws_instance" "workload" {
  ...
  user_data = <<-USERDATA
    ...
    aws s3 cp "s3://diploma-scripts/bootstrapMcmirrorSmall.sh" "/tmp/bootstrapMcmirrorSmall.sh"
    chmod +x "/tmp/bootstrapMcmirrorSmall.sh"
    /tmp/bootstrapMcmirrorSmall.sh
  USERDATA

  tags = {
    Name = "mcmirror-small-benchmark"
  }
}
