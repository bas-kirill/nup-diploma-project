...
resource "aws_iam_role" "bench_role" {
  name               = "mcmirror-large-bench-ec2-role"
...
resource "aws_iam_instance_profile" "bench_profile" {
  name = "mcmirror-large-bench-instance-profile"
  role = aws_iam_role.bench_role.name
}
...
resource "aws_subnet" "workload_public" {
  ...
  tags = {
    Name = "mcmirror-large-public-subnet"
  }
}
...
resource "aws_instance" "workload" {
  ...
  user_data = <<-USERDATA
    ...
    aws s3 cp "s3://diploma-scripts/bootstrapMcmirrorLarge.sh" "/tmp/bootstrapMcmirrorLarge.sh"
    chmod +x "/tmp/bootstrapMcmirrorLarge.sh"
    /tmp/bootstrapMcmirrorLarge.sh
  USERDATA

  tags = {
    Name = "mcmirror-large-benchmark"
  }
}
