...
resource "aws_iam_role" "bench_role" {
  name               = "s4cmdsync-maxdatasetsize-bench-ec2-role"
...
resource "aws_iam_instance_profile" "bench_profile" {
  name = "s4cmdsync-maxdatasetsize-bench-instance-profile"
  role = aws_iam_role.bench_role.name
}
...
resource "aws_vpc" "workload" {
  ...
  tags = {
    Name = "s4cmdsync-maxdatasetsize-vpc"
  }
}
...
resource "aws_internet_gateway" "workload" {
  ...
  tags = {
    Name = "s4cmdsync-maxdatasetsize-igw"
  }
}
...
resource "aws_subnet" "workload_public" {
  ...
  tags = {
    Name = "s4cmdsync-maxdatasetsize-public-subnet"
  }
}
...
resource "aws_route_table" "workload_public" {
  ...
  tags = {
    Name = "s4cmdsync-maxdatasetsize-public-rt"
  }
}
...
resource "aws_security_group" "workload_sg" {
  name        = "s4cmdsync-maxdatasetsize-sg"
  vpc_id      = aws_vpc.workload.id
...
resource "aws_cloudwatch_log_group" "workload_logs" {
  name              = "/bench/s4cmdsync-maxdatasetsize"
...
resource "aws_instance" "workload" {
  ...
  user_data = <<-USERDATA
    ...
    aws s3 cp "s3://diploma-scripts/bootstrapS4cmdsyncMaxDatasetSize.sh" "/tmp/bootstrapS4cmdsyncMaxDatasetSize.sh"
    chmod +x "/tmp/bootstrapS4cmdsyncMaxDatasetSize.sh"
    nohup /tmp/bootstrapS4cmdsyncMaxDatasetSize.sh > /var/log/bootstrap.log 2>&1 &
  USERDATA

  tags = {
    Name = "s4cmdsync-maxdatasetsize-benchmark"
  }
}
