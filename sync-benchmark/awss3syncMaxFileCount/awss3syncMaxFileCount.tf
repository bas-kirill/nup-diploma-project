...
resource "aws_iam_role" "bench_role" {
  name               = "awss3sync-maxfilecount-bench-ec2-role"
...
resource "aws_iam_instance_profile" "bench_profile" {
  name = "awss3sync-maxfilecount-bench-instance-profile"
  role = aws_iam_role.bench_role.name
}
...
resource "aws_vpc" "workload" {
  ...
  tags = { Name = "awss3sync-maxfilecount-vpc" }
}

resource "aws_internet_gateway" "workload" {
  vpc_id = aws_vpc.workload.id
  tags   = { Name = "awss3sync-maxfilecount-igw" }
}
...
resource "aws_subnet" "workload_public" {
  ...
  tags = { Name = "awss3sync-maxfilecount-public-subnet" }
}

resource "aws_route_table" "workload_public" {
  vpc_id = aws_vpc.workload.id
  tags   = { Name = "awss3sync-maxfilecount-public-rt" }
}
...
resource "aws_security_group" "workload_sg" {
  name        = "awss3sync-maxfilecount-sg"
  vpc_id      = aws_vpc.workload.id
...
resource "aws_cloudwatch_log_group" "workload_logs" {
  name              = "/bench/awss3sync-maxfilecount"
...
resource "aws_instance" "workload" {
  ...
  user_data = <<-USERDATA
    ...
    aws s3 cp "s3://diploma-scripts/bootstrapAwss3syncMaxFileCount.sh" "/tmp/bootstrapAwss3syncMaxFileCount.sh"
    chmod +x "/tmp/bootstrapAwss3syncMaxFileCount.sh"
    /tmp/bootstrapAwss3syncMaxFileCount.sh
  USERDATA

  tags = { Name = "awss3sync-maxfilecount-benchmark" }
}
