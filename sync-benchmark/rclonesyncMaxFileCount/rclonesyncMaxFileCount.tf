...
resource "aws_iam_role" "bench_role" {
  name               = "rclonesync-maxfilecount-bench-ec2-role"
...
resource "aws_iam_instance_profile" "bench_profile" {
  name = "rclonesync-maxfilecount-bench-instance-profile"
  role = aws_iam_role.bench_role.name
}
...
resource "aws_vpc" "workload" {
  ...
  tags = {
    Name = "rclonesync-maxfilecount-vpc"
  }
}
...
resource "aws_internet_gateway" "workload" {
  ...
  tags = {
    Name = "rclonesync-maxfilecount-igw"
  }
}
...
resource "aws_subnet" "workload_public" {
  ...
  tags = {
    Name = "rclonesync-maxfilecount-public-subnet"
  }
}
...
resource "aws_route_table" "workload_public" {
  ...
  tags = {
    Name = "rclonesync-maxfilecount-public-rt"
  }
}
...
resource "aws_security_group" "workload_sg" {
  name        = "rclonesync-maxfilecount-sg"
  vpc_id      = aws_vpc.workload.id
...
resource "aws_cloudwatch_log_group" "workload_logs" {
  name              = "/bench/rclonesync-maxfilecount"
...
resource "aws_instance" "workload" {
  ...
  user_data = <<-USERDATA
    ...
    aws s3 cp "s3://diploma-scripts/bootstrapRclonesyncMaxFileCount.sh" "/tmp/bootstrapRclonesyncMaxFileCount.sh"
    chmod +x "/tmp/bootstrapRclonesyncMaxFileCount.sh"
    /tmp/bootstrapRclonesyncMaxFileCount.sh
  USERDATA

  tags = {
    Name = "rclonesync-maxfilecount-benchmark"
  }
}
