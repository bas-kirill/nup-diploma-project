...
S3_BUCKET_SANDBOX="s3://diploma-rclonesync-maxfilecount-benchmark"
...
rclonesyncMaxFileCount() {
  ...
  rclone sync "/tmp/prewarming/" "aws:diploma-rclonesync-maxfilecount-benchmark" \
    --checksum \
    --transfers 16 \
    --checkers 16 \
  ...
  for i in {1..10}; do
    ...
    perf record -F 99 -g --call-graph=dwarf -o "$perf_data_file" -- \
      rclone sync "$datasets_folder" "aws:diploma-rclonesync-maxfilecount-benchmark" \
        --checksum \
        --transfers 16 \
        --checkers 16 \
        --s3-upload-concurrency 4 \
